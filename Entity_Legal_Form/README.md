...
* can leverage the [string split function](https://jena.apache.org/documentation/query/library-propfunc.html) from the underlying Jena Fuseki, to split a string using a delimiter, see [line 104]( https://github.com/SEMICeu/Taxonomy/blob/master/Entity_Legal_Form/SPARQL-query-for-ELF-v1.5.rq#L104)
...
* links the countries of formation of the ELF codes with the countries in the Publications Office [Country and territories Authority Table](https://op.europa.eu/en/web/eu-vocabularies/dataset/-/resource?uri=http://publications.europa.eu/resource/dataset/country) by using the matching retrieved in [lines 96-98](https://github.com/SEMICeu/Taxonomy/blob/master/Entity_Legal_Form/SPARQL-query-for-ELF-v1.5.rq#L96-L98) with the filter in [line 120](https://github.com/SEMICeu/Taxonomy/blob/master/Entity_Legal_Form/SPARQL-query-for-ELF-v1.5.rq#L120)
...
The example below shows how the code [JLR3](https://github.com/SEMICeu/Taxonomy/blob/master/Entity_Legal_Form/2023-09-28-elf-code-list-v1.5.csv#L111-L112) in multiple languages with transliteration is transformed. In particular, it is possible to see how the skos-xl:labels allow to associate the labels with the respective abbreviations.
...
| ipl - Inconsistent Preferred Labels	Finds resources with more then one prefLabel per language.	FAIL (1) | The code [X0SD](2023-09-28-elf-code-list-v1.5.csv#L338-L339) is  not valid in SKOS because it has 2 preferred labels (it is valid accordingly to GLEIF), the problem is resolved manually by changing the preferred label in alternative label. |
